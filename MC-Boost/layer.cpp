...
Layer::Layer(Layer_Properties &props)
{
    this->mu_a = props.mu_a;
    this->mu_s = props.mu_s;
    this->mu_t = mu_a + mu_s;
    
    albedo = mu_s/(mu_s + mu_a);
    g = props.anisotropy;
    this->refractive_index = props.refractive_index;
    
    this->depth_start = props.start_depth;
    this->depth_end = props.end_depth;
}
...
/// Returns the ansitropy value after checking to see if the photon
/// might be within an absorber.
double Layer::getAnisotropy(const boost::shared_ptr<Vector3d> photonVector)
{
