...
%
% DESCRIPTION:
%       gradientSpect gradientFD calculates the gradient of an
%       n-dimensional input matrix using the finite-difference method. For
%       one-dimensional inputs, the gradient is always computed along the
%       non-singletone dimension. For higher dimensinal inputs, the
%       gradient for singleton dimensions is returned as 0. For elements in
%       the centre of the grid, the gradient is computed using centered
%       finite-differences. For elements on the edge of the grid, the
%       gradient is computed using forward or backward finite-differences.
%       The order of accuracy of the finite-difference approximation is
%       controlled by accuracy_order (default = 2). The calculations are
%       done using sparse multiplication, so the input matrix is always
